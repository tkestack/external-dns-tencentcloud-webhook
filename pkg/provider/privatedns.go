...
func (p *TencentCloudProvider) privateZoneRecords() ([]*endpoint.Endpoint, error) {
	...
	for _, recordList := range recordMap {
		...
		if first.Weight != nil {
			ep.SetProviderSpecificProperty(PropertyWeight, strconv.FormatInt(*first.Weight, 10))
		}
		if first.MX != nil && *first.MX > 0 {
			ep.SetProviderSpecificProperty(PropertyMX, strconv.FormatInt(*first.MX, 10))
		}
...
func (p *TencentCloudProvider) createPrivateZoneRecord(zone *privatedns.PrivateZone, ep *endpoint.Endpoint, target string) error {
	request := privatedns.NewCreatePrivateZoneRecordRequest()
	request.ZoneId = common.StringPtr(*zone.ZoneId)
	request.RecordType = common.StringPtr(ep.RecordType)
	request.RecordValue = common.StringPtr(target)
	request.SubDomain = common.StringPtr(getSubDomain(*zone.Domain, ep))
	if ep.RecordTTL.IsConfigured() {
		request.TTL = common.Int64Ptr(int64(ep.RecordTTL))
	}

	// Weight
	if w, ok := getProviderSpecificInt64(ep, PropertyWeight); ok {
		request.Weight = common.Int64Ptr(w)
	}

	// MX priority
	if mx, ok := getProviderSpecificInt64(ep, PropertyMX); ok {
		request.MX = common.Int64Ptr(mx)
	}

	// Remark
	if remark, ok := getProviderSpecificString(ep, PropertyRemark); ok {
