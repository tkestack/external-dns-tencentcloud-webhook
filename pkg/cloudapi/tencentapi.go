...
func NewTencentAPIService(region string, rate int, credential common.CredentialIface, internetEndpoint bool) *defaultTencentAPIService {
	tencentAPIService := &defaultTencentAPIService{
		RetryDefault:      3,
		TaskCheckInterval: 3 * time.Second,
		ClientSetService:  NewTencentClientSetService(region, rate, credential, internetEndpoint),
	}
