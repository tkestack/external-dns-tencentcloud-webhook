...
// NewTencentClientSetService creates a client set with the given credential.
// The credential can be a static *common.Credential or an OIDC provider result.
func NewTencentClientSetService(region string, rate int, credential common.CredentialIface, internetEndpoint bool) *defaultTencentClientSetService {
	...
	p.privateDnsClient, _ = privatedns.NewClient(credential, region, privatednsProf)

	dnsPodProf := profile.NewClientProfile()
	if !internetEndpoint {
		dnsPodProf.HttpProfile.Endpoint = "dnspod.internal.tencentcloudapi.com"
	}
	p.dnsPodClient, _ = dnspod.NewClient(credential, region, dnsPodProf)
